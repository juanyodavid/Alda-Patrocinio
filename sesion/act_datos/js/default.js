...
function autocompletar(){
    ...
    const editCliente = document.querySelector('#cliente_edit');    
    let indexFocus = -1;
    ...
    editCliente.addEventListener('input', function(){
        ...
        // conexión a BD
        httpRequest('controller1.php?cliente_edit=' + tipoCliente, function(){
            ...
            arreglo.forEach(item => {
                // console.log(item);
                item = item.toString();
                if(item.substr(0, tipoCliente.length) == tipoCliente){
                    const elementoLista = document.createElement('div');
                    elementoLista.innerHTML = `<strong>${item.substr(0, tipoCliente.length)}</strong>${item.substr(tipoCliente.length)}`;
                    elementoLista.addEventListener('click', function(){
                        editCliente.value = this.innerText;
                        cerrarLista();
                        return false;
                    });
                    divList.appendChild(elementoLista);
    ...
    editCliente.addEventListener('keydown', function(e){
        const divList = document.querySelector('#' + this.id + '-lista-autocompletar');
        let items;
